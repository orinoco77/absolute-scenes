...
export function createFontComparison(fontFamilies, options = {}) {
  ...
  return fontFamilies.map(fontFamily => {
    const fontInfo = Object.values(BOOK_FONTS).find(
...
function generateRecommendations(fontSize, lineHeight, fontInfo) {
  ...
  if (fontSize < 10) {
    recommendations.push(
      'Consider increasing font size to 10-12pt for better readability'
    );
  } else if (fontSize > 14) {
    recommendations.push(
      'Font size might be too large for comfortable reading'
    );
  }

  if (lineHeight < 1.2) {
    recommendations.push(
      'Increase line height to 1.2-1.4 for better line spacing'
    );
  } else if (lineHeight > 1.8) {
    recommendations.push(
      'Line height might be too loose, consider reducing to 1.4-1.6'
    );
  }

  if (fontInfo && fontInfo.category === 'sans-serif') {
    recommendations.push(
      'Consider using a serif font for body text - easier on the eyes for long reading'
    );
  }

  if (!fontInfo || fontInfo.quality === 'standard') {
    recommendations.push(
      'Consider upgrading to a premium book font for professional appearance'
    );
  }
  ...
  const recommendations = {
    print: {
      trade: {
        fontSize: 11,
        lineHeight: 1.3,
        note: 'Standard for 6x9 trade paperbacks'
      },
      'mass-market': {
        fontSize: 9,
        lineHeight: 1.2,
        note: 'Compact size requires smaller font'
      },
      hardcover: {
        fontSize: 12,
        lineHeight: 1.4,
        note: 'Larger format allows comfortable reading'
      },
      'large-print': {
        fontSize: 16,
        lineHeight: 1.5,
        note: 'Enhanced readability for vision assistance'
      }
    },
    ebook: {
      fontSize: 12,

...
describe('CharacterEditor Component', () => {
  ...
  test('renders no character selected message when no character provided', () => {
    render(<CharacterEditor character={null} template={mockTemplate} onCharacterUpdate={mockOnCharacterUpdate} />);
    
    expect(screen.getByText('No Character Selected')).toBeInTheDocument();
    expect(screen.getByText('Select a character from the list to edit their information, or create a new character.')).toBeInTheDocument();
  });

  test('renders character editor with character data', () => {
    render(<CharacterEditor character={mockCharacter} template={mockTemplate} onCharacterUpdate={mockOnCharacterUpdate} />);
    ...
    expect(screen.getByDisplayValue('Main character notes')).toBeInTheDocument();
    // Use a more specific selector for the current avatar
    expect(screen.getByText('👨', { selector: '.current-avatar' })).toBeInTheDocument();
  });
  ...
  test('updates character name', () => {
    ...
    expect(mockOnCharacterUpdate).toHaveBeenCalledWith('char-1', { name: 'Jane Smith' });
  });
  ...
  test('updates character role', () => {
    ...
    expect(mockOnCharacterUpdate).toHaveBeenCalledWith('char-1', { role: 'Antagonist' });
  });
  ...
  test('updates character notes', () => {
    ...
    fireEvent.change(notesTextarea, { target: { value: 'Updated character notes' } });
    
    expect(mockOnCharacterUpdate).toHaveBeenCalledWith('char-1', { notes: 'Updated character notes' });
  });
  ...
  test('auto-saves description after delay', async () => {
    ...
    fireEvent.change(descriptionTextarea, { target: { value: 'A brave and noble warrior' } });
    
    // Fast-forward time to trigger auto-save
    jest.advanceTimersByTime(1000);
    
    await waitFor(() => {
      expect(mockOnCharacterUpdate).toHaveBeenCalledWith('char-1', { description: 'A brave and noble warrior' });
    });
  ...
  test('calculates word count correctly', () => {
    ...
    render(<CharacterEditor character={characterWithLongDescription} template={mockTemplate} onCharacterUpdate={mockOnCharacterUpdate} />);
  ...
  test('handles character prop changes', () => {
    const { rerender } = render(
      <CharacterEditor character={mockCharacter} template={mockTemplate} onCharacterUpdate={mockOnCharacterUpdate} />
    );
    ...
    rerender(
      <CharacterEditor character={updatedCharacter} template={mockTemplate} onCharacterUpdate={mockOnCharacterUpdate} />
    );
  ...
  test('resets state when character becomes null', () => {
    ...
    rerender(
      <CharacterEditor character={null} template={mockTemplate} onCharacterUpdate={mockOnCharacterUpdate} />
    );

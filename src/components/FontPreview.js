...
function FontPreview({
  ...
  const [previewText, setPreviewText] = useState(
    sampleText ||
      `The quick brown fox jumps over the lazy dog. This sample text demonstrates how the font appears in both regular and italic styles, and shows readability across different character combinations.`
  );

  // Get font information
  const fontKey = Object.keys(BOOK_FONTS).find(
    key => BOOK_FONTS[key].name === selectedFont
  );
  ...
  const sampleTexts = {
    readability:
      'The quick brown fox jumps over the lazy dog. This sample text demonstrates how the font appears in both regular and italic styles, and shows readability across different character combinations.',
    literary:
      'It was the best of times, it was the worst of times, it was the age of wisdom, it was the age of foolishness. In a hole in the ground there lived a hobbit. Not a nasty, dirty, wet hole filled with the ends of worms and an oozy smell.',
    academic:
  ...
  return (
    <div className="font-preview">
      <div className="font-preview-header" style={{ marginBottom: '10px' }}>
        ...
        {fontInfo && (
          <div style={{ fontSize: '12px', color: '#666' }}>
            <span>
              <strong>Style:</strong> {fontInfo.characteristics}
            </span>
            <span style={{ marginLeft: '15px' }}>
              <strong>Best for:</strong> {fontInfo.bestFor.join(', ')}
            </span>
          </div>
        )}
      </div>
      ...
      <div style={{ marginBottom: '10px' }}>
        <label style={{ fontSize: '12px', color: '#666', marginRight: '10px' }}>
          Sample text:
        </label>
        <select
          value="custom"
        ...
        {fontInfo && (
          <div style={{ marginLeft: 'auto', fontSize: '11px', color: '#666' }}>
            ...
            <span
              style={{
                color:
                  fontInfo.quality === 'premium'
                    ? '#4caf50'
                    : fontInfo.quality === 'high'
                      ? '#2196f3'
                      : '#666',
                fontWeight: 'bold',
